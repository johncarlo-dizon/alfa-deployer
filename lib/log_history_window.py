import tkinter as tk
from tkinter import ttk, messagebox

from log_manager import read_full_log, clear_full_log

# ---------------------------------------------------------------------------
# Theme constants (kept in sync with the main dashboard's dark palette)
# ---------------------------------------------------------------------------
COLOR_BG = "#14161f"
COLOR_PANEL_ALT = "#11131b"
COLOR_PANEL_ALT_TEXT = "#c9cde0"
COLOR_FIELD = "#242838"
COLOR_BORDER = "#2e3346"
COLOR_TEXT = "#e7e9f5"
COLOR_TEXT_MUTED = "#8b90a8"
COLOR_ACCENT = "#9b6bff"
COLOR_DANGER = "#f0546a"
COLOR_DANGER_DARK = "#c73752"

FONT_LABEL = ("Segoe UI", 9, "bold")
FONT_BTN = ("Segoe UI", 8)
FONT_MONO_SMALL = ("Consolas", 8)


def _init_ttk_style():
    style = ttk.Style()
    try:
        style.theme_use("clam")
    except tk.TclError:
        pass
    for orient in ("Vertical", "Horizontal"):
        style.configure(
            f"{orient}.TScrollbar",
            background=COLOR_FIELD, troughcolor=COLOR_PANEL_ALT,
            bordercolor=COLOR_PANEL_ALT, arrowcolor=COLOR_TEXT_MUTED,
            relief="flat", borderwidth=0, gripcount=0,
        )
        style.map(
            f"{orient}.TScrollbar",
            background=[("active", COLOR_ACCENT), ("pressed", COLOR_ACCENT)],
            arrowcolor=[("active", COLOR_TEXT)],
        )


def _style_button(btn, bg, fg="white", hover=None):
    hover = hover or bg
    btn.configure(bg=bg, fg=fg, activebackground=hover, activeforeground=fg,
                  relief="flat", bd=0, cursor="hand2", highlightthickness=0)
    btn.bind("<Enter>", lambda e: btn.configure(bg=hover))
    btn.bind("<Leave>", lambda e: btn.configure(bg=bg))
    return btn


def show_log_history_window(parent_root):
    win = tk.Toplevel(parent_root)
    win.title("Deployment Logs History")
    win.configure(bg=COLOR_BG)
    _init_ttk_style()

    window_width = 720
    window_height = 440
    center_x = int((win.winfo_screenwidth() / 2) - (window_width / 2))
    center_y = int((win.winfo_screenheight() / 2) - (window_height / 2))
    win.geometry(f"{window_width}x{window_height}+{center_x}+{center_y}")

    top_frame = tk.Frame(win, bg=COLOR_PANEL_ALT)
    top_frame.pack(fill="x")
    accent_strip = tk.Frame(top_frame, bg=COLOR_ACCENT, width=3)
    accent_strip.pack(side="left", fill="y")

    tk.Label(top_frame, text="FULL DETAILED DEPLOYMENT LOGS", font=FONT_LABEL,
             bg=COLOR_PANEL_ALT, fg=COLOR_PANEL_ALT_TEXT, padx=8, pady=5).pack(side="left", fill="x")

    text_frame = tk.Frame(win, bg=COLOR_BG, highlightthickness=1, highlightbackground=COLOR_BORDER)
    text_frame.pack(fill="both", expand=True, padx=8, pady=6)

    log_text = tk.Text(text_frame, font=FONT_MONO_SMALL, wrap="word",
                        bg=COLOR_FIELD, fg=COLOR_TEXT, relief="flat", bd=0,
                        highlightthickness=0, padx=6, pady=6)
    scroll = ttk.Scrollbar(text_frame, orient="vertical", command=log_text.yview)
    log_text.configure(yscrollcommand=scroll.set)

    log_text.pack(side="left", fill="both", expand=True)
    scroll.pack(side="right", fill="y")

    def load_content():
        log_text.config(state="normal")
        log_text.delete("1.0", tk.END)
        log_text.insert("1.0", read_full_log())
        log_text.config(state="disabled")
        log_text.see(tk.END)

    def do_clear():
        if messagebox.askyesno("Clear Log History", "This will permanently delete the saved log history file. Continue?"):
            if clear_full_log():
                load_content()
            else:
                messagebox.showerror("Error", "Could not clear the log file.")

    clear_btn = tk.Button(top_frame, text="🗑️ Clear History", font=FONT_BTN, command=do_clear, padx=8, pady=2)
    _style_button(clear_btn, COLOR_DANGER, hover=COLOR_DANGER_DARK)
    clear_btn.pack(side="right", padx=6, pady=4)

    load_content()